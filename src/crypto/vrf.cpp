...
std::optional<VrfProof> vrf_prove(const Bytes& private_key_32, const PubKey32& pubkey, const Bytes& transcript) {
  auto sig = ed25519_sign(transcript, private_key_32);
  if (!sig.has_value()) return std::nullopt;
  VrfProof p;
  p.proof.assign(sig->begin(), sig->end());
  codec::ByteWriter w;
  w.bytes(Bytes{'S', 'C', '-', 'V', 'R', 'F', '-', 'O', 'U', 'T', '-', 'V', '5'});
...
bool vrf_verify(const PubKey32& pubkey, const Bytes& transcript, const VrfProof& p) {
  if (p.proof.size() != 64) return false;
  Sig64 sig{};
  std::copy(p.proof.begin(), p.proof.end(), sig.begin());
  if (!ed25519_verify(transcript, sig, pubkey)) return false;
  codec::ByteWriter w;
  w.bytes(Bytes{'S', 'C', '-', 'V', 'R', 'F', '-', 'O', 'U', 'T', '-', 'V', '5'});
  w.bytes_fixed(pubkey);
  w.bytes(p.proof);
